...
def generate_main_js(data):
    ...
    for i, item in enumerate(journey):
        journey_content_kr += f"journey{i+1}Title: '{item['title_kr']}',\n        journey{i+1}Desc: '{item['desc_kr']}',\n        "
        journey_content_en += f"journey{i+1}Title: '{item['title_en']}',\n        journey{i+1}Desc: '{item['desc_en']}',\n        "
...
const content = {{
    KR: {{
        ...
        {journey_content_kr}workLabel: 'Work & Publications',
        workDesc: '{work[0]["desc_kr"] if work else ""}',
    ...
    EN: {{
        ...
        journeyLabel: 'My Journey',
        {journey_content_en}workLabel: 'Work & Publications',
        workDesc: '{work[0]["desc_en"] if work else ""}',
        contactLabel: 'Get in Touch',
...
document.addEventListener('keydown', (e) => {{
    ...
    if (!document.getElementById('cmdPalette').classList.contains('show')) {{
        ...
        if (e.key === '2') scrollToSection('journey');
        if (e.key === '3') scrollToSection('work');
        if (e.key === '4') scrollToSection('contact');
        if (e.key === '5') scrollToSection('re');
